...
int CNewParticleEffect::DrawModel( int flags )
{
	...
	{
		C_BasePlayer *pCameraObject = C_BasePlayer::GetLocalPlayer();
		if( pCameraObject->GetViewEntity() )
		{
			pCameraObject = ToBasePlayer( pCameraObject->GetViewEntity() );
		}
		...
		Render( nViewRecursionLevel, pRenderContext, ( flags & STUDIO_TRANSPARENCY ) ? IsParticleTwoPass() : false, pCameraObject );
		pRenderContext->MatrixMode( MATERIAL_MODEL );
