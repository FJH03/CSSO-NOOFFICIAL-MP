...
int ComputeActualMipCount( const TexDimensions_t& actualDims, unsigned int nFlags )
{
	...
	if ( nFlags & TEXTUREFLAGS_ALL_MIPS )
	{
	...
	if ( nFlags & TEXTUREFLAGS_MOST_MIPS )
	{
		bMostMips = true;
	}
	...
	if ( ( !IsOpenGL() && !bForceTextureAllMips ) || bMostMips )
	{
		...
		int nNumMipLevels = 1;
		int w = actualDims.m_nWidth;
		int h = actualDims.m_nHeight;
