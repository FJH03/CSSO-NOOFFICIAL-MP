...
BEGIN_VS_SHADER_FLAGS( Engine_Post_dx9, "Engine post-processing effects (software anti-aliasing, bloom, color-correction", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			bool bDesaturateEnable = bToolMode && ( params[DESATURATEENABLE]->GetIntValue() != 0 ) && g_pHardwareConfig->SupportsPixelShaders_2_b() && IsWindows();
			...
			int aaEnabled = false;
			if ( IsGameConsole() || ( g_pHardwareConfig->SupportsShaderModel_3_0() ) )
			{
